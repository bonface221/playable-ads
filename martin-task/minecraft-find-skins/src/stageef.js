const StageEF = () => {
	...
	if (appObj.time_current - appObj.time_old > 25) {
		...
		// Checking if the stage where all items are visible is done
		// we do so by checking if the interactive of one element is on which will represent all the other elements
		// Here we also grab the inventory elements for use in the final card.
		if (appMc["skin_1_bgQ"]) {
			if (appMc["skin_1_bgQ"].interactive === true) {
				if (appObj.gameLevel === 2) {
					console.log(appObj.checkInventory());
					if (appObj.checkInventory() === 16) {
						if (appObj.getSkinsInInventory().length !== 0) {
							appObj.skinsInInventory = appObj.getSkinsInInventory();
							console.log(appObj.skinsInInventory);
						} else {
						...
						appMc["skin_1_bgQ"].interactive = false;
					}
				}
			}
		}
