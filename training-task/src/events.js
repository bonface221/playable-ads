const onDragStart = () => {
	appMc.mcRect.dragging = true;
};

const onDragEnd = () => {
	appMc.mcRect.dragging = false;
};

const onDragMove = (e) => {
	if (appMc.mcRect.dragging === true) {
		appMc.mcRect.x = e.data.getLocalPosition(appMc.mcWorldCamera).x;
		appMc.mcRect.y = e.data.getLocalPosition(appMc.mcWorldCamera).y;
	}
};
