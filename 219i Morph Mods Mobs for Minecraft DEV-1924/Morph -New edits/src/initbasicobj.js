const InitBasicObj = () => {
	...
	appMc.mcNextBtn = new createSprite({
		...
		tex: "next-btn",
		y: 0,
		x: appMc.mcTapToSelectBtn.width / 2 + 15,
		anchor: [0, 0.5],
		scale: 0.5,
	});
...
function showStage2(id) {
	TweenLite.killTweensOf(appMc.mcTakeTheSkinText.scale);
	appMc.mcTakeTheSkinText.scale.set(1);


	
	gsap.from(appMc.mcSelectedSkinText.scale, 0.5, {
		x: 0.0,
		y: 0.0,
		overwrite: "none",
		ease: "Back.easeOut",
	...
	gsap.from(appMc.mcBackArrow.scale, 0.5, {
		x: 0.0,
		y: 0.0,
		overwrite: "none",
		ease: "Back.easeOut",
	...
	gsap.to(appMc.mcTakeTheSkinText.scale, 0.5, {
		x: 0.95,
		y: 0.95,
		overwrite: "none",
		ease: "Sine.easeInOut",
	...
	appSounds.cracking.play();
	appMc.mcLightening = new createSequence({
		p: appMc.mcStage2Container,
		tex: "lightening-sequence",
		data: aDataFrames,
		prefix: "frame_",
		frame: 0,
		totalFrame: 8,
...
function gsapAnimation(btn) {
	gsap.killTweensOf(appMc[btn].scale);
	gsap.to(appMc[btn].scale, 0.1, {
		overwrite: "none",
		x: 0.6,
		y: 0.6,
		ease: "Sine.easeOut",
